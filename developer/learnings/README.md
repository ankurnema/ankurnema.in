...
| [004a-useeffect-setstate-lint.md](004a-useeffect-setstate-lint.md) | Prompt 004a | `react-hooks/set-state-in-effect` flags `setState` called directly in `useEffect`; fix: close overlays via `onClick` handlers on nav links, not via `useEffect` watching `pathname` |
| [004a-dropdown-hover-gap.md](004a-dropdown-hover-gap.md) | Prompt 004a | Dropdown closes before cursor reaches panel — attach hover handlers to wrapper div (not button), add 200ms `closeTimerRef` grace period; guard `relatedTarget` against null |
| [tools-playwright-fadein-screenshot.md](tools-playwright-fadein-screenshot.md) | Tools | `fullPage: true` screenshots show blank sections where FadeInSection is used — IntersectionObserver never fires; fix: scroll to section, take viewport screenshot; also: `scrollIntoView()` in evaluate can trigger link navigation — prefer `window.scrollTo(0, N)` |
| [design-floating-header-scroll.md](design-floating-header-scroll.md) | Design pass | Scroll-triggered floating header: outer `sticky top-0` (never changes), inner div toggles `mx-4 mt-4 rounded-2xl` via `scrolled` state — never put a top offset on the outer sticky element if initial state should be flush |
| [design-3d-button-shadow.md](design-3d-button-shadow.md) | Design pass | 3D buttons on web need soft glow not hard bottom edge: three layers — `inset 0 1px 0` top highlight (convex feel) + large soft ambient glow + tiny drop shadow; hard `0 4px 0 0 color` reads as "brick" on backlit screens |
| [design-equal-height-cards.md](design-equal-height-cards.md) | Design pass | Equal-height grid cards need `h-full` on BOTH the FadeInSection wrapper (motion.div) AND the card root element; CSS `columns` (masonry) intentionally produces variable heights — use CSS `grid` instead when equal-height rows are required |
