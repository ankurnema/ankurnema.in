...
export default function RootLayout({
  ...
  return (
    <html lang="en" className={`${inter.variable} ${dmSans.variable}`}>
      <body className="bg-brand-canvas text-brand-charcoal font-sans">
        <header className="w-full px-6 py-4 bg-brand-surface border-b border-brand-slate/20">
          <Link href="/" aria-label="Ankur Nema — home">
            <LogoText />
          </Link>
        </header>
        <main>{children}</main>
        <footer className="w-full px-6 py-6 bg-brand-surface border-t border-brand-slate/20 text-brand-slate text-sm">
